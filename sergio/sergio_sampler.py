...
def grn_sergio(
    *,
    ...
    g, # graph
    effect_sgn, # indicates 
    toporder, # 
    n_vars, # 
    # specific inputs required in config 
    b,  # basal reproduction rate of master regulators 
    k_param,  # function: k parameter (interaction strength) values
    k_sign_p,  # function: 
    hill,  # float: hill coefficients of nonlinear interactions / determines non-linearity of relationships
    decays,  # float: gene-wise decay rates 
    noise_params,  # float: scale of stochastic noise process
    cell_types,  # int: number of cell types
    noise_type='dpd', #
    sampling_state=15, #
    dt=0.01, #
    # technical noise NOTE: not used for DiBS
    ...
    # NOTE if we knew real graph, could force signs
